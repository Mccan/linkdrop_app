...
const _deviceAliasOverrides = 'ls_device_alias_overrides';
...
class PersistenceService {
  ...
  static Future<PersistenceService> initialize({
    ...
    final showToken = prefs.getString(_showToken)!;
    final defaultAlias = buildDefaultAlias(showToken);

    if (prefs.getString(_aliasKey) == null) {
      await prefs.setString(_aliasKey, defaultAlias);
      await prefs.setBool(_aliasModifiedKey, false);
    }

    // Stop using generated/random aliases. If user did not customize alias, force deterministic default.
    final isAliasModified = prefs.getBool(_aliasModifiedKey) ?? false;
    if (!isAliasModified && prefs.getString(_aliasKey) != defaultAlias) {
      await prefs.setString(_aliasKey, defaultAlias);
    }
  ...
  Map<String, String> getDeviceAliasOverrides() {
    final raw = _prefs.getString(_deviceAliasOverrides);
  ...
  Future<void> setDeviceAliasOverrides(Map<String, String> overrides) async {
    if (overrides.isEmpty) {
      await _prefs.remove(_deviceAliasOverrides);
      return;
    }

    await _prefs.setString(_deviceAliasOverrides, jsonEncode(overrides));
  }
  ...
  String getAlias() {
    final storedAlias = _prefs.getString(_aliasKey);
    if (storedAlias != null && storedAlias.trim().isNotEmpty) {
      return storedAlias;
    }

    final showToken = getShowToken();
    return buildDefaultAlias(showToken);
  }
