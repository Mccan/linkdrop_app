...
class ReceiveController {
  ...
  Future<void> _prepareUploadHandler({
    ...
    final aliasOverrides = server.ref.read(deviceAliasOverridesProvider);
    final localAliasOverride = aliasOverrides[dto.info.fingerprint]?.trim();
    final favoriteAlias = server.ref.read(favoritesProvider).firstWhereOrNull((e) => e.fingerprint == dto.info.fingerprint)?.alias;
    final resolvedSenderAlias = (localAliasOverride != null && localAliasOverride.isNotEmpty)
        ? localAliasOverride
        : (favoriteAlias ?? dto.info.alias);
      ...
      if (quickSave) {
        ...
        Future.delayed(Duration.zero, () {
          ...
          Navigator.of(Routerino.context).pushAndRemoveUntil(
            MaterialPageRoute(
              builder: (context) => const LinkDropHomePage(initialTab: HomeTab.receive, appStart: false),
            ),
            (route) => false,
          );
...
void _cancelBySender(ServerUtils server) {
  ...
  // 无论当前在哪个页面，都尝试返回到首页
  // 如果已经在 ProgressPage，状态更新后会触发倒计时自动返回
  // 如果在 ReceiveSessionPage 或 ReceiveOptionsPage，需要关闭它
  // 使用 addPostFrameCallback 确保在正确的 UI 上下文中执行
