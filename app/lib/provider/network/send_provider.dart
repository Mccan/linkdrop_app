...
class SendNotifier extends Notifier<Map<String, SendSessionState>> {
  ...
  Future<void> startSession({
    ...
    // 判断是否为纯文本消息（单条文本，内容在preview中）
    final isTextMessage = files.length == 1 && files.first.fileType == FileType.text && files.first.bytes != null;

    final requestState = SendSessionState(
      sessionId: sessionId,
      remoteSessionId: null,
      background: background,
      status: isTextMessage ? SessionStatus.sending : SessionStatus.waiting,
      target: target,
    ...
    if (!background) {
      ...
      Routerino.context.push(
        () => ProgressPage(showAppBar: false, closeSessionOnClose: true, sessionId: sessionId),
    ...
    if (fileMap.isEmpty) {
      // receiver has nothing selected (usually text message or declined)
      await _saveSendHistory(requestState, allowDeliveredMessagesWithoutUpload: true);
      ...
      // 对于文本消息，短暂延迟后自动返回首页，给用户一个"已送达"的视觉反馈
      if (state[sessionId]?.background == false) {
        final isTextMessage = requestState.files.values.every((f) => f.isMessage);
        if (isTextMessage) {
          // 文本消息：延迟1.5秒让用户看到"已完成"状态
        ...
        if (Routerino.context.mounted) {
          ...
          // ignore: use_build_context_synchronously, unawaited_futures
          Navigator.of(Routerino.context).pushAndRemoveUntil(
            MaterialPageRoute(
              builder: (context) => const LinkDropHomePage(initialTab: HomeTab.send, appStart: false),
            ),
            (route) => false,
          );
        }
  ...
  /// 保存发送历史记录
  Future<void> _saveSendHistory(
    SendSessionState sessionState, {
    bool allowDeliveredMessagesWithoutUpload = false,
  }) async {
    final target = sessionState.target;
    final timestamp = DateTime.now();
    final localAliasOverride = ref.read(deviceAliasOverridesProvider)[target.fingerprint];
    final receiverAlias = (localAliasOverride != null && localAliasOverride.trim().isNotEmpty) ? localAliasOverride : target.alias;
    final entries = buildSendHistoryEntries(
      sessionState: sessionState,
      timestamp: timestamp,
      receiverAlias: receiverAlias,
      entryIdBuilder: (_) => _uuid.v4(),
      allowDeliveredMessagesWithoutUpload: allowDeliveredMessagesWithoutUpload,
    );
      ...
      await ref
          ...
          .dispatchAsync(
            AddSendHistoryEntryAction(
              ...
              receiverAlias: entry.receiverAlias,
